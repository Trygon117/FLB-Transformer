...
def fetch_mapped_context_kernel(
    ...
    active_cells_ptr,
    num_active_cells, num_deps, seq_len, num_total_cells,
    ...
    for i in range(num_deps):
        ...
        valid_mask = is_real_cell[:, None] & (target_idx_2d >= 0)
        grid_data = tl.load(grid_ptrs, mask=valid_mask, other=0.0)
        
        x_mask = is_real_cell[:, None] & (target_idx_2d == -2)
...
def run_fetch_kernel(x, stacked_grids, routing_map, port_map, config, active_cells_tensor, gathered_out):
    num_deps = len(config.dependencies)
    ...
    fetch_mapped_context_kernel[grid](
        ...
        # The slice is 4D: (Deps, Cells, Batch, Dim)
        gathered_out.stride(1), # stride_gathered_cell  (Index 1)
        gathered_out.stride(0), # stride_gathered_dep   (Index 0)
        gathered_out.stride(2), # stride_gathered_batch (Index 2)
        gathered_out.stride(3), # stride_gathered_dim   (Index 3)
